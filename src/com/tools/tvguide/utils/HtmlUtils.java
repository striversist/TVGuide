...
public class HtmlUtils {
    public static Document getDocument(String url, CacheControl control)
            throws IOException {
    ...
    public static Document getDocument(String url, String charset,
            List<BasicNameValuePair> pairs, CacheControl control)
            throws IOException {
        String html = getHTMLString(url, charset, pairs, control);
        Document doc = Jsoup.parse(html);
        return doc;
    }

    public static String getHTMLString(String url, String charset,
            List<BasicNameValuePair> pairs, CacheControl control)
            throws IOException {
        String html = null;
        ...
        if (control == CacheControl.Memory) {
            ...
            TimestampString tString = AppEngine.getInstance()
                    .getDiskCacheManager().getTimestampString(key);
        ...
        if (html == null) {
            ...
            getter.setHeader("Referer",
                    "http://www.tvmao.com/program/CCTV-CCTV1-w1.html");
            for (int i = 0; i < 2; ++i) {
                if (pairs != null) {
            ...
            if (control == CacheControl.Memory) {
                ...
                AppEngine.getInstance().getDiskCacheManager()
                        .setString(key, html);
            } else if (control == CacheControl.DiskToday) {
                TimestampString tString = new TimestampString(
                        System.currentTimeMillis(), html);
                AppEngine.getInstance().getDiskCacheManager()
                        .setTimestampString(key, tString);
            }
        }
        return html;
    }
